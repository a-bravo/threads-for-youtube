...
describe('store', () => {
  test('addSubmissionAction', () => {
    ...
    expect(store.state.submissions.s_1).toBe(undefined);

    const submission = { data: { name: 's_1' } };
    store.addSubmissionAction(submission);

    expect(store.state.submissionList).toHaveLength(1);
    expect(store.state.submissions.s_1.data.name).toBe('s_1');
    expect(store.state.submissions.s_1.comments).toHaveLength(0);
    expect(store.state.submissions.s_1.loading).toBe(false);
    expect(store.state.submissions.s_1.error).toBe(false);
  });

  describe('comments', () => {
    beforeEach(() => {
      store.addSubmissionAction({ data: { name: 's_1' } });
    });
    ...
    describe('addCommentAction', () => {
      test('top level comment', () => {
        ...
        expect(store.state.submissions.s_1.comments).toHaveLength(1);
    ...
    test('addComments', () => {
      ...
      // comment added to submission.comments
      expect(store.state.submissions.s_1.comments).toHaveLength(2);
